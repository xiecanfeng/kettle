...
public class ScriptValuesModController extends BaseController{

	@ResponseBody
	@RequestMapping(method=RequestMethod.POST, value="/tree")
	protected void tree(@RequestParam String graphXml, @RequestParam String stepName,ModelMap model) throws Exception {
	...
	protected void getVariables(@RequestParam String graphXml, @RequestParam String stepName,  @RequestParam String scriptName,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.TRANS_CODEC);
		App app = (App) model.get("app");
		TransMeta transMeta = (TransMeta) codec.decode(graphXml,app);
	...
	protected void testData(@RequestParam String graphXml, @RequestParam String stepName,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.TRANS_CODEC);
		App app = (App) model.get("app");
		TransMeta transMeta = (TransMeta) codec.decode(graphXml,app);
	...
	protected void test(@RequestParam String graphXml, @RequestParam String stepName, @RequestParam String rowGenerator,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.TRANS_CODEC);
		App app = (App) model.get("app");
		TransMeta transMeta = (TransMeta) codec.decode(graphXml,app);
