...
public class JobGraphController extends BaseController{
	
	@ResponseBody
	@RequestMapping(method=RequestMethod.POST, value="/engineXml")
	protected void engineXml(@RequestParam String graphXml,ModelMap model) throws Exception {
		App app = (App) model.get("app");
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
	...
	protected void entries(@RequestParam String graphXml,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		App app = (App) model.get("app");
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
	...
	protected void getSQL(@RequestParam String graphXml,ModelMap model) throws Exception {
		...
		List<SQLStatement> stats = getJobSQLProgress.run(repository);
		JSONArray jsonArray = new JSONArray();
	...
	protected void save(@RequestParam String graphXml,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		App app = (App) model.get("app");
		JobMeta jobMeta = (JobMeta) codec.decode(StringEscapeHelper.decode(graphXml),app);
		Repository repository = app.getRepository();
//		Repository repository = App.getInstance().getRepository();
	...
	protected void newJobEntry(@RequestParam String graphXml, @RequestParam String pluginId, @RequestParam String name,ModelMap model) throws Exception {
		App app = (App) model.get("app");
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
		...
		if (jobPlugin != null) {
			...
			Element e = encoder.encodeStep(jge,app);
	...
	protected void newHop(@RequestParam String graphXml, @RequestParam String fromLabel, @RequestParam String toLabel,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		App app = (App) model.get("app");
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
	...
	protected void ftpputtest(@RequestParam String graphXml, @RequestParam String stepName,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		App app = (App) model.get("app");
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
	...
	protected void ftpputtestremotedir(@RequestParam String graphXml, @RequestParam String stepName,ModelMap model) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.JOB_CODEC);
		App app = (App) model.get("app");
		JobMeta jobMeta = (JobMeta) codec.decode(graphXml,app);
...
//			ftpputClient = new FTPClient  (InetAddress.getByName(servername), Const.toInt(serverport, 22), username);
	...
	protected void initRun(@RequestParam String graphXml,ModelMap model) throws Exception {
		...
		JobExecutionConfiguration executionConfiguration = app.getJobExecutionConfiguration();
...
//		JobExecutionConfiguration executionConfiguration = App.getInstance().getJobExecutionConfiguration();
	...
	protected void run(@RequestParam String graphXml, @RequestParam String executionConfiguration,ModelMap model) throws Exception {
	    ...
	    JobExecutor jobExecutor = JobExecutor.initExecutor(jobExecutionConfiguration, jobMeta,app);
