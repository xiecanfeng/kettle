 JobLogEntryPanel = Ext.extend(Ext.Panel, {
	...
	initComponent: function() {
		...
		var entryLogTable = Ext.decode(root.getAttribute('entryLogTable'));
		...
		wConnection.setValue(entryLogTable.connection);
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{
				...
				anchor: '-10',
				name: 'schema',
				value: entryLogTable.schema
			},{
				xtype:'textfield',
				fieldLabel: '日志表',
				anchor: '-10',
				name: 'table',
				value: entryLogTable.table
			},{
				xtype:'textfield',
				fieldLabel: '日志记录过时时间(天)',
				anchor: '-10',
				name: 'timeout_days',
				value: entryLogTable.timeout_days
			}]
		});
		...
		var grid = new Ext.grid.EditorGridPanel({
			...
			store: new Ext.data.JsonStore({
				...
				data: entryLogTable.fields
			})
		});
		this.items = [form, grid];
		
		this.getEntryLogTable = function(){
			var values = form.getForm().getFieldValues();
			entryLogTable.connection = values.connection;
			entryLogTable.schema = values.schema;
			entryLogTable.table = values.table;
			entryLogTable.timeout_days = values.timeout_days;
			
			return entryLogTable;
		};
		
		JobLogChannelPannel.superclass.initComponent.call(this);
	}

});

Ext.reg('JobLogEntry', JobLogEntryPanel);