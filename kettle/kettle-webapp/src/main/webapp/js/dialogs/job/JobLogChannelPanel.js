 JobLogChannelPannel = Ext.extend(Ext.Panel, {
	...
	initComponent: function() {
		...
		wConnection.setValue(channelLogTable.connection);
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{
				...
				anchor: '-10',
				name: 'schema',
				value: channelLogTable.schema
			},{
				xtype:'textfield',
				fieldLabel: '日志表',
				anchor: '-10',
				name: 'table',
				value: channelLogTable.table
			},{
				xtype:'textfield',
				fieldLabel: '日志记录过时时间(天)',
				anchor: '-10',
				name: 'timeout_days',
				value: channelLogTable.timeout_days
			}]
		});
		...
		this.getChannelLogTable = function(){
			var values = form.getForm().getFieldValues();
			channelLogTable.connection = values.connection;
			channelLogTable.schema = values.schema;
			channelLogTable.table = values.table;
			channelLogTable.timeout_days = values.timeout_days;
...
Ext.reg('JobLogChannel', JobLogChannelPannel);