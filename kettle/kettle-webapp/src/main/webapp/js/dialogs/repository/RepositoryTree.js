	...
	initComponent: function() {
		var menu = new Ext.menu.Menu({
			items: [{
				...
				text: '新建流程图', scope: this, handler: this.newFlowsheet 
			}, '-', {
	    ...
	    this.tbar = [{
			...
			menu: [{
				...
				text: '日志监控', scope: this, handler: this.logManage
			}]
		}, {
			text: '参数管理',
			menu: [{
//				text: '数据库参数', scope: this, handler: this.ParamManage
				text: '数据库参数', scope: this, handler: this.DatabaseManage
			},{
				text: '公共参数', scope: this, handler: this.ParamManage
			}]
		}, {
			text: '关于', 
			menu: [{
	...
	newFlowsheet: function(){
		window.open('mxgraph/index.html');
	},
	...
	rename:function(){
		...
		Ext.Msg.prompt('名称'+name, '请输入新名称:', function(btn, text){
		    if (btn == 'ok' && text != ''){
		    	Ext.getBody().mask('正在修改名称，请稍后...');
		    	
		    	Ext.Ajax.request({
					url: GetUrl('repository/rename.do'),
					method: 'POST',
					params: {id: id, newName: text},
					success: function(response) {
						try {
							/*var path = Ext.decode(response.responseText).message;
							node.reload(function() {
	...
	DatabaseManage: function() {
		/*var databaseDialog = new DatabaseParamDialog();
		databaseDialog.show(null, function() {
			databaseDialog.initReposityDatabase(null);
		});*/
		var dialog = new DatabaseParamDialog();
		dialog.show();
	},
	
	ParamManage: function() {
		var dialog = new ParamDialog();
		dialog.show();
	},
	
	logManage: function() {
		var dialog = new LogManageDialog();
		dialog.show();
	},
