TransLogTransPanel = Ext.extend(Ext.Panel, {
	...
	initComponent: function() {
		...
		wConnection.setValue(transLogTable.connection);
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{
					...
					anchor: '-10',
					name: 'schema',
					value: transLogTable.schema
				},{
					xtype:'textfield',
					fieldLabel: '日志表',
					anchor: '-10',
					name: 'table',
					value: transLogTable.table
				},{
					xtype:'textfield',
					fieldLabel: '日志记录间隔时间(秒)',
					anchor: '-10',
					name: 'interval',
					value: transLogTable.interval
				},{
					xtype:'textfield',
					fieldLabel: '日志记录过时时间(天)',
					anchor: '-10',
					name: 'timeout_days',
					value: transLogTable.timeout_days
				},{
					xtype:'textfield',
					fieldLabel: '在内存中保存的日志行数限制',
					anchor: '-10',
					name: 'size_limit_lines',
					value: transLogTable.size_limit_lines
				}
			]
		});
		...
		this.getTransLogTable = function(){
			var values = form.getForm().getFieldValues();
			transLogTable.connection = values.connection;
			transLogTable.schema = values.schema;
			transLogTable.table = values.table;
			transLogTable.size_limit_lines = values.size_limit_lines;
			transLogTable.interval = values.interval;
			transLogTable.timeout_days = values.timeout_days;
