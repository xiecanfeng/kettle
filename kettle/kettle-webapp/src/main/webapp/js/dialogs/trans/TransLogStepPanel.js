TransLogStepPanel = Ext.extend(Ext.Panel, {
	...
	initComponent: function() {
		...
		wConnection.setValue(stepLogTable.connection);
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{
				...
				anchor: '-10',
				name: 'schema',
				value: stepLogTable.schema
			},{
				xtype:'textfield',
				fieldLabel: '日志表',
				anchor: '-10',
				name: 'table',
				value: stepLogTable.table
			},{
				xtype:'textfield',
				fieldLabel: '日志记录过时时间(天)',
				anchor: '-10',
				name: 'timeout_days',
				value: stepLogTable.timeout_days
			}]
		});
		...
		this.getStepLogTable = function(){
			var values = form.getForm().getFieldValues();
			stepLogTable.connection = values.connection;
			stepLogTable.schema = values.schema;
			stepLogTable.table = values.table;
			stepLogTable.timeout_days = values.timeout_days;
