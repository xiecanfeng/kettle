BaseGraph = Ext.extend(Ext.Panel, {
	...
	getDatabaseStore: function() {
		...
		Ext.Ajax.request({
			url: GetUrl('repository/getDatabase.do'),
			async:false,
			method: 'POST',
			params: {},
			success: function(response) {
				
				var databases = response.responseText;
				if(databases!=null && databases!=''){
					root.setAttribute('databases',databases);
					return root;
				}
			},
			failure: failureResponse
		...
		/************************************************/
		/*var s = '[{"name":"mysql","type":"MYSQL","access":"0","hostname":"127.0.0.1","databaseName":"mysql","port":"3306","username":"root","password":"root","supportBooleanDataType":true,"supportTimestampDataType":true,"extraOptions":[],"usingConnectionPool":"N","initialPoolSize":"5","maximumPoolSize":"10","partitioned":"N","partitionInfo":[]}]';
		root.setAttribute('databases',s);*/
