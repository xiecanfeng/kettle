...
public abstract class BaseGraphCodec implements GraphCodec {
	...
	public Element encodeCommRootAttr(AbstractMeta meta, Document doc) throws UnknownParamException {
	    ...
	    JSONObject jsonObject = new JSONObject();
	    jsonObject.put( "connection", channelLogTable.getConnectionName()==null?"":channelLogTable.getConnectionName());
	    jsonObject.put( "schema", channelLogTable.getSchemaName()==null?"":channelLogTable.getSchemaName());
	    jsonObject.put( "table", channelLogTable.getTableName()==null?"":channelLogTable.getTableName());
	    jsonObject.put( "timeout_days", channelLogTable.getTimeoutInDays()==null?"":channelLogTable.getTimeoutInDays());
	...
	public void decodeCommRootAttr(mxCell root, AbstractMeta meta,App app) throws KettleException, JsonParseException, JsonMappingException, IOException {
		Repository repository = app.getRepository();
//		Repository repository = App.getInstance().getRepository();
		meta.setRepository(repository);
		meta.setMetaStore(app.getMetaStore());
