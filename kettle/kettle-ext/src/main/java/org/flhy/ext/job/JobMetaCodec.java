...
public class JobMetaCodec extends BaseGraphCodec {
	...
	public String encode(AbstractMeta meta,App app) throws Exception {
		...
		try {
	    	...
	    	for ( int i = 0; i < vars.size(); i++ ) {
	    		...
	    		if ( !varname.startsWith( Const.INTERNAL_VARIABLE_PREFIX ) && Const.indexOfString( varname, jobMeta.listParameters() ) < 0 ) {
	    	...
	    	for ( String varname : Const.INTERNAL_JOB_VARIABLES ) {
		    	String value = jobMeta.getVariable( varname );
		    ...
		    jsonObject.put("fields", fields);
		    e.setAttribute("entryLogTable", jsonObject.toString());
			
		    e.setAttribute("is_batchId_passed", jobMeta.isBatchIdPassed() ? "Y" : "N");
		    e.setAttribute("shared_object_file", jobMeta.getSharedObjectsFile());
			...
			for(int i=0; i<jobMeta.nrJobEntries(); i++) {
				...
				Object cell = graph.insertVertex(parent, null, stepEncoder.encodeStep(jge,app), p.x, p.y, PropsUI.STEP_SIZE, PropsUI.STEP_SIZE, "icon;image=" + image);
				cells.put(jge, cell);
			}
	...
	public AbstractMeta decode(String graphXml,App app) throws Exception {
		...
		decodeCommRootAttr(root, jobMeta, app);
		jobMeta.setJobversion(root.getAttribute("job_version"));
		int jobStatus = Const.toInt(root.getAttribute("job_status"), -1) + 1;
		if(jobStatus >= 0)
