...
public class TransMetaCodec extends BaseGraphCodec {
	...
	public String encode(AbstractMeta meta,App app) throws Exception {
		...
		try {
			...
			String[] params = transMeta.listParameters();
			
			JSONArray paramArray = new JSONArray();
			for(String param : params){
				JSONObject paramObject = new JSONObject(); 
				paramObject.put("name", param);
				paramObject.put("default_value", transMeta.getParameterDefault(param) == null?"":transMeta.getParameterDefault(param));
				paramObject.put("description", transMeta.getParameterDescription(param) == null?"":transMeta.getParameterDescription(param));
				//paramObject.put("value", transMeta.getParameterValue(param) == null?"":transMeta.getParameterValue(param));
				paramArray.add(paramObject);
			}
			e.setAttribute("parameters", paramArray.toString());
			
			e.setAttribute("max_date_connection", String.valueOf(transMeta.getMaxDateConnection()==null?"":transMeta.getMaxDateConnection()));
			e.setAttribute("max_date_table", String.valueOf(transMeta.getMaxDateTable()==null?"":transMeta.getMaxDateTable()));
			e.setAttribute("max_date_field", String.valueOf(transMeta.getMaxDateField()==null?"":transMeta.getMaxDateField()));
			e.setAttribute("max_date_offset", String.valueOf(transMeta.getMaxDateOffset()==0.0?"":transMeta.getMaxDateOffset()));
			e.setAttribute("max_date_diff", String.valueOf(transMeta.getMaxDateDifference()==0.0?"":transMeta.getMaxDateDifference()));
		   
			transMeta.listVariables();
		    ...
		    jsonObject.put( "connection", transLogTable.getConnectionName()==null?"":transLogTable.getConnectionName());
		    jsonObject.put( "schema", transLogTable.getSchemaName()==null?"":transLogTable.getSchemaName());
		    jsonObject.put( "table", transLogTable.getTableName()==null?"":transLogTable.getTableName());
		    jsonObject.put( "size_limit_lines", transLogTable.getLogSizeLimit()==null?"":transLogTable.getLogSizeLimit());
		    jsonObject.put( "interval", transLogTable.getLogInterval()==null?"":transLogTable.getLogInterval());
		    jsonObject.put( "timeout_days", transLogTable.getTimeoutInDays()==null?"":transLogTable.getTimeoutInDays());
		    ...
		    jsonObject.put( "connection", stepLogTable.getConnectionName()==null?"":stepLogTable.getConnectionName());
		    jsonObject.put( "schema", stepLogTable.getSchemaName()==null?"":stepLogTable.getSchemaName());
		    jsonObject.put( "table", stepLogTable.getTableName()==null?"":stepLogTable.getTableName());
		    jsonObject.put( "timeout_days", stepLogTable.getTimeoutInDays()==null?"":stepLogTable.getTimeoutInDays());
		    ...
		    jsonObject = new JSONObject();
		    jsonObject.put( "connection", performanceLogTable.getConnectionName()==null?"":performanceLogTable.getConnectionName());
		    jsonObject.put( "schema", performanceLogTable.getSchemaName()==null?"":performanceLogTable.getSchemaName());
		    jsonObject.put( "table", performanceLogTable.getTableName()==null?"":performanceLogTable.getTableName());
		    jsonObject.put( "interval", performanceLogTable.getLogInterval()==null?"":performanceLogTable.getLogInterval());
		    jsonObject.put( "timeout_days", performanceLogTable.getTimeoutInDays()==null?"":performanceLogTable.getTimeoutInDays());
		    fields = new JSONArray();
		    ...
		    jsonObject = new JSONObject();
		    jsonObject.put( "connection", metricsLogTable.getConnectionName()==null?"":metricsLogTable.getConnectionName());
		    jsonObject.put( "schema", metricsLogTable.getSchemaName()==null?"":metricsLogTable.getSchemaName());
		    jsonObject.put( "table", metricsLogTable.getTableName()==null?"":metricsLogTable.getTableName());
		    jsonObject.put( "timeout_days", metricsLogTable.getTimeoutInDays()==null?"":metricsLogTable.getTimeoutInDays());
		    fields = new JSONArray();
		    ...
		    jsonObject.put("table", transMeta.getMaxDateTable()==null?"":transMeta.getMaxDateTable());
		    jsonObject.put("field", transMeta.getMaxDateField()==null?"":transMeta.getMaxDateField());
		    jsonObject.put("offset", transMeta.getMaxDateOffset()==0.0?"":transMeta.getMaxDateOffset());
		    jsonObject.put("maxdiff", transMeta.getMaxDateDifference()==0.0?"":transMeta.getMaxDateDifference());
			...
			for(int i=0; i<list.size(); i++) {
				...
				if (stepMeta.getStepMetaInterface() instanceof HasRepositoryInterface) {
					((HasRepositoryInterface) stepMeta.getStepMetaInterface()).setRepository(app.getRepository());
				}
	...
	public AbstractMeta decode(String graphXml,App app) throws Exception {
		...
		decodeCommRootAttr(root, transMeta, app);
		transMeta.setTransstatus(Const.toInt( root.getAttribute( "trans_status" ), -1 ) + 1);
	    ...
	    //页面数据中局部参数
	    JSONArray params = JSONArray.fromObject(root.getAttribute("parameters"));
	    Object[] array = params.toArray();
		for (int i = 0; i < array.length; i++) {
			Map object = (Map) array[i];
			String name = object.get("name")!=null?object.get("name").toString():"";
			String default_value = object.get("default_value")!=null?object.get("default_value").toString():"";
			transMeta.setVariable( name, Const.NVL( default_value, "" ) );
		}
