...
public class TransExecutor implements Runnable {
	...
	private TransExecutor(TransExecutionConfiguration transExecutionConfiguration, TransMeta transMeta,App app) {
	...
	public static synchronized TransExecutor initExecutor(TransExecutionConfiguration transExecutionConfiguration, TransMeta transMeta,App app) {
		TransExecutor transExecutor = new TransExecutor(transExecutionConfiguration, transMeta,app);
...
//		        	log.logError( trans.getName() + ": preparing transformation execution failed", e );
				...
				carteObjectId = Trans.sendToSlaveServer( transMeta, executionConfiguration, app.getRepository(), app.getMetaStore() );
				...
				try {
					...
					try {
						Trans.cleanupCluster(app.getLog(), transSplitter);
					} catch (Exception ee) {
...
//				}
				...
				Trans.monitorClusteredTransformation(app.getLog(), transSplitter, null);
				
				
				Result result = Trans.getClusteredTransformationResult(app.getLog(), transSplitter, null);
				errCount = result.getNrErrors();
			}
			
		} catch(Exception e) {
			e.printStackTrace();
			app.getLog().logError("执行失败！", e);
		} finally {
			finished = true;
		}
	}
